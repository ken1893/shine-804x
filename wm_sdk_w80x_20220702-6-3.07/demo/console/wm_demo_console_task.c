...
static u32 TASK2_task_stk[DEMO_TASK_SIZE];		//设置任务栈的起始地址
tls_os_task_t Task2_HandHdle;				    //创建任务句柄

static u32 TASK3_task_stk[TASK_STK_SIZE];		//设置任务栈的起始地址
tls_os_task_t Task3_HandHdle;				    //创建任务句柄

extern int btfuc_control(void);
...
void Task3_lvx_test(void * num)
{
	for(;;)
	{
		tls_os_time_delay(100);	//延时     3 / 6ms
		io_condis();            // 
	}
}
...
void CreateIOTask(void)
{
	...
	tls_os_task_create(Task2_HandHdle,
		 	 	 "Task2_lvx_test",
		 	  	 Task2_lvx_test,
		 	  	 (void *)param2,
				 (void *)&TASK2_task_stk,
	...
	tls_os_task_create(Task3_HandHdle,
		 	 	 "Task3_lvx_test",
		 	  	 Task3_lvx_test,
		 	  	 (void *)param3,
				 (void *)&TASK3_task_stk,
