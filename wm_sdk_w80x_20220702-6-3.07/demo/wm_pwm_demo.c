...
static void pwm_isr_callback(void)
{
    ...
    else
    {
        ...
        if(status&0x00000020)              //ÉÏÉýÑØÖÐ¶Ï
        {
 ...
 */
 ...
 /*
  * 通过uart0发送t-pwm=(1,250,99,4,0),示波器量 PB01 可以测到 250Hz，占空比约为39%(99/255)的波形
  * 通过修改duty调节电压
  * */
int pwm_demo(u8 channel, u16 freq, u8 duty, u8 mode, u8 num)
{
    ...
    {
        pwm_demo_multiplex_config(channel);
		tls_pwm_stop(channel_used);       // stop
    }
    else
    {
        return WM_FAILED;
    ...
    switch (mode)
    {
        case WM_PWM_OUT_MODE_INDPT:    // mode 4
